...
session_start();
    ...
    $adminServiceId = $_GET['adminServiceId'];
    $adminId = $_GET['id'];
    

    

        $sql = "SELECT * FROM adminservice WHERE id = :id LIMIT 1";   
    ...
    if (isset($_POST['submit'])) {     
     ...
     // Handle file upload
        ...
        if (move_uploaded_file($_FILES['receipt']['tmp_name'], $filePath)) {
            ...
            $stmtUpdate->execute(['receipt' => $filePath,'status' => $status,'id' => $adminId]);

            
                                
            
            header('location: earner?admintask=success');
        } else {
...
<body class="bg-gray-50">
    <div class="flex min-h-screen">
            ...
            <div class="p-6">
                <div style="float:right;">
                    <div class="flex items-center justify-between ml-auto">
                        <h1 class="text-xl font-bold text-dark">Social Boost</h1>
                    ...
                    <div class="flex items-center space-x-4 ml-auto">
                        <div class="relative">
                            <button class="text-gray-600 hover:text-primary">
                                ...
                                <span
                                    class="absolute -top-1 -right-1 w-4 h-4 flex items-center justify-center bg-red-500 text-white text-xs rounded-full">3</span>
                            </button>
                        </div>
                        <div class="relative">
                            <button class="text-gray-600 hover:text-primary">
                                ...
                                <span
                                    class="absolute -top-1 -right-1 w-4 h-4 flex items-center justify-center bg-primary text-white text-xs rounded-full">5</span>
                            </button>
                        </div>
                        <button
                ...
                <div class="max-w-2xl mx-auto bg-white p-6 rounded-2xl shadow-xl">
                    ...
                    <div class="mb-6">
                        ...
                        <div class="flex items-center bg-gray-50 border border-gray-300 rounded-md px-3 py-2">
                            <input id="socialUrl" type="text" readonly value="<?php echo $row->socialUrl; ?>"
                                class="flex-1 bg-transparent text-sm text-gray-600 outline-none">
                            <button onclick="copyLink('socialUrl')" type="button"
                    ...
                    <!-- Admin Post Url -->
                    <div class="mb-6">
                        <label class="block text-sm font-medium text-gray-700 mb-2">Post Link</label>
                        <div class="flex items-center bg-gray-50 border border-gray-300 rounded-md px-3 py-2">
                            <input id="postUrl" type="text" readonly value="<?php echo $row->postUrl; ?>"
                                class="flex-1 bg-transparent text-sm text-gray-600 outline-none">
                            <button onclick="copyLink('postUrl')" type="button"
                                class="text-blue-600 hover:text-blue-800 ml-3">
                                <i class="fas fa-copy"></i>
                            </button>
                        </div>
                    ...
                    <!-- Toast -->
                    <div id="toast"
                        class="fixed bottom-4 right-4 bg-green-500 text-white text-sm px-4 py-2 rounded shadow opacity-0 transition-opacity duration-500">
                        Link copied!
                    </div>
        ...
        // Separate at commas into an array
        $items = explode(",", $text);

        // Output each item in an <li>
        echo "<ol>";
        foreach ($items as $item) {
            $trimmed = trim($item); // remove spaces/newlines
            if (!empty($trimmed)) {
                echo "<li>$trimmed</li>";
            }
        }
                    ...
                    <form action="" method="post" enctype="multipart/form-data">
                        <div class="mb-6">
                            ...
                            <label for="screenshot"
                                ...
                                <input id="screenshot" name="receipt" type="file" class="hidden" accept="image/*"
                        ...
                        <input type="hidden" name="adminServiceId" value="<?php echo $row-> id ?>"/>

                        <!-- Submit Button -->
                        <button type="submit" name="submit"
                        
                            class="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-md font-medium text-sm transition duration-200">
                            Submit Task
                        </button>
                    </form>
