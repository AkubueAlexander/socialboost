<?php
    ...
    $sql = "SELECT COUNT(*) AS total_count FROM socialorder";
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
    $totalCount = $stmt->fetchColumn();

    $pending = 'pending';
    $sqlPending = "SELECT COUNT(*) AS total_count FROM socialorder WHERE  status = :pending";
    $stmtPending = $pdo->prepare($sqlPending);
    $stmtPending->execute(['pending' => $pending]);
    $pendingCount = $stmtPending->fetchColumn();

    $inProgress = 'In Progress';
    $sqlInProgress = "SELECT COUNT(*) AS total_count FROM socialorder WHERE  status = :inProgress";
    $stmtInProgress = $pdo->prepare($sqlInProgress);
    $stmtInProgress->execute(['inProgress' => $inProgress]);
    $inProgressCount = $stmtInProgress->fetchColumn();

    $completed = 'completed';
    $sqlCompleted = "SELECT COUNT(*) AS total_count FROM socialorder WHERE status = :completed";
    $stmtCompleted = $pdo->prepare($sqlCompleted);
    $stmtCompleted->execute([ 'completed' => $completed]);    
            ...
            <main class="flex-1 overflow-y-auto p-4 md:p-6 bg-gray-50">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                    ...
                    <div class="flex justify-center lg:col-span-3">
                        ...
                        <div class="w-2/3 bg-white rounded-xl shadow-sm p-5">
